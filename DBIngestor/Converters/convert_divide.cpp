...
convert_divide::convert_divide() {
    string converterName = "CONV_DIVIDE";
...
Converter * convert_divide::clone() {
    return new convert_divide;
}

bool convert_divide::execute(DBDataSchema::DType thisDType, void* value) {
    assert(value != NULL);
    
	//apply divide to the value
    switch (thisDType) {
        ...
        default:
            DBIngestor_error("Converter Error: CONV_DIVIDE does not handle the datatype - ONLY INTS, FLOAT AND DOUBLE SUPPORTED\n", NULL);
            break;
    }
