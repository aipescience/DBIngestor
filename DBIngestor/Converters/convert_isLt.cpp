...
convert_islt::convert_islt() {
    string converterName = "CONV_ISLT";
...
Converter * convert_islt::clone() {
    return new convert_islt;
}

bool convert_islt::execute(DBDataSchema::DType thisDType, void* value) {
    assert(value != NULL);
    
	//apply islt to the value
    switch (thisDType) {
        ...
        default:
            DBIngestor_error("Converter Error: CONV_islt does not handle the datatype - ONLY INTS, FLOAT AND DOUBLE SUPPORTED\n", NULL);
            break;
    }
