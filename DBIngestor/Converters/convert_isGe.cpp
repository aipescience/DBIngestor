...
convert_isge::convert_isge() {
    string converterName = "CONV_ISGE";
...
Converter * convert_isge::clone() {
    return new convert_isge;
}

bool convert_isge::execute(DBDataSchema::DType thisDType, void* value) {
    assert(value != NULL);
    
	//apply isge to the value
    switch (thisDType) {
        ...
        default:
            DBIngestor_error("Converter Error: CONV_isge does not handle the datatype - ONLY INTS, FLOAT AND DOUBLE SUPPORTED\n", NULL);
            break;
    }
