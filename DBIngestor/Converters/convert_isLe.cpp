...
convert_isle::convert_isle() {
    string converterName = "CONV_ISLE";
...
Converter * convert_isle::clone() {
    return new convert_isle;
}

bool convert_isle::execute(DBDataSchema::DType thisDType, void* value) {
    assert(value != NULL);
    
	//apply isle to the value
    switch (thisDType) {
