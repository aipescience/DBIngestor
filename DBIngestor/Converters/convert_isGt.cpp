...
convert_isgt::convert_isgt() {
    string converterName = "CONV_ISGT";
...
Converter * convert_isgt::clone() {
    return new convert_isgt;
}

bool convert_isgt::execute(DBDataSchema::DType thisDType, void* value) {
    assert(value != NULL);
    
	//apply isgt to the value
    switch (thisDType) {
        ...
        default:
            DBIngestor_error("Converter Error: CONV_ISGT does not handle the datatype - ONLY INTS, FLOAT AND DOUBLE SUPPORTED\n", NULL);
            break;
    }
