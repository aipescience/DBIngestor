...
#ifndef DBIngestor_DBODBC_h
#define DBIngestor_DBODBC_h

namespace DBServer {
    
    /*! \class DBODBC
     \brief DBODBC communication class
     
     This class implements all the DBAbstractor methods needed for communicating
     through an ODBC connection.
     */
    class DBODBC : public DBAbstractor {
    private:
        /*! \var SQLHENV * odbcEnv
        ...
        /*! \brief Binds the buffer pointers to the ODBC Handle (prepared statement) 
         ...
         Binds all the buffers in the prepared statement structure with the ODBC statement
         handle.*/
        int bindStatement(void* preparedStatement);

        int bindColStatement(void* preparedStatement);
    ...
    public:
        ...
        /*! \brief connects to a database server. 
         ...
         socket + ;UID;PWD.*/
		virtual int connect(string usr, string pwd, string host, string port, string socket);
		...
		virtual DBDataSchema::Schema * getSchema(string database, string table);
